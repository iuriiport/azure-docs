...
Enabling the Azure Firewall on [Azure Key Vault](/azure/key-vault/general/network-security) blocks access from Azure Automation Account and cloud jobs for that service. Access will be blocked even when the firewall exception to allow trusted Microsoft services is enabled, as Automation is not a part of the trusted services list. With an enabled firewall, access can only be made for runbooks executed on a Hybrid Runbook Worker and a [virtual network service endpoint](/azure/key-vault/general/overview-vnet-service-endpoints). However, when you enable only the Private link for Key Vault, Azure Automation Account loses access to the Key Vault. Even if you enable a Private link for Automation account, it will allow access from Hybrid Runbook Worker to Azure Automation service and not from Automation account to the Key Vault.
